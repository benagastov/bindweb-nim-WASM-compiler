...
/* NOTE: the upstream repo version of this file carries a "WebAssembly entry
 * shim" (bindweb_wasm_entry asm("main") calling NimMain) intended for native
 * --noMain builds. It is REMOVED in this libpack: the in-browser pipeline
 * keeps Nim's generated main and rewrites it to the 2-arg form crt1 expects,
 * so a second main here would be a duplicate symbol at link time. */
