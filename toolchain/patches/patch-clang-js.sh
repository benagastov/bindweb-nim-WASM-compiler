#!/bin/bash
# =============================================================================
# patch-clang-js.sh  --  apply the in-browser Nim->wasm pipeline fix
#                        (-fno-common injected into the clang.js worker)
# =============================================================================
#
...
#
# 2. A weak raise() stub.  wasi-libc *declares* raise() in <signal.h> but
#    does not implement it (WASI has no signals); Nim's system module calls
#    raise() in its signal path. A weak no-op definition resolves the link
#    without duplicate-symbol errors. In this rebuild the stub lives in the
#    web pipeline (web/src/pipeline.js appends it to the C files).
#
# 3. A 2-arg main rewrite.  wasi crt1.o calls main with signature
#    (i32,i32)->i32, i.e. main(argc,argv). Nim emits a 3-arg
#    main(argc,args,env); the mismatch produces an INVALID wasm module.
#    The web pipeline's C-source cleaner rewrites it to the 2-arg form and
#    declares env as a NULL local.
#
# Parts 2 and 3 live in web/src/pipeline.js. Part 1 (this script) edits
# clang.js, because the actual clang `-cc1` invocation is built inside the
# base64-embedded web worker in clang.js.
#
...
# no effect on the real bug.
#
# Run:  toolchain/patches/patch-clang-js.sh                 # patches web/vendor/clang/clang.js
#       toolchain/patches/patch-clang-js.sh path/to/clang.js [more.js ...]
...
echo "Done. The weak raise() stub and the 2-arg main rewrite live in"
echo "web/src/pipeline.js (the C-source fixup stage)."
